...
>>>>>>> 38d576654a0f1f66f6dd0f99607c369e8079ecb9
    ...
    public function getWhere($where_default = false, $where_solid = false, $relation = false)
    {
        ...
        if (!empty($param['seach_all']) && empty($param['like_seach'])) {
            ...
            $seach_all    = [];
            foreach ($seach_all_ys['name'] as $key => $value) {
     ...
     * @param $name 名称
     * @param $value 参数
     * @Author: 苹果  SpringYang <ceroot@163.com>
     */
     ...
     * @param  string 字段是否添加表名前缀
     * @return array  表名称.字段  数组
     */
    public function getTablePrefixFields($model_list = '', $prefix = true)
    {
        if (empty($model_list)) {
    ...
    public function getModelClass($name = '', $layer = 'model', $base = 'Base', $appendSuffix = false, $common = 'common')
    {
        try {
            ...
            if (isset($this->info['modelpath']) && !empty($this->info['modelpath'])) {
    ...
    {
        $path  = isset($this->info['basemodelpath']) && !empty($this->info['basemodelpath']) ? $this->info['basemodelpath'] . DIRECTORY_SEPARATOR . $layer : 'app\common\\' . $layer;
        $class = $path . '\\' . $base;
    ...
    public function checkModelAttr($fields = false, $data = [])
    {
        ...
        foreach ($fields as $key => $attr) {
            ...
            switch ($attr['auto_time']) {
                case '1':
                    if (empty($data['id']) && !empty($attr['auto_rule'])) {
                        // 新增
            ...
            foreach ($list as $k => $data) {
                foreach ($data as $key => $val) {
                    if (isset($attrList[$key])) {
                        ...
                        } elseif ('date' == $type && is_int($val)) {
                            // 日期型
                            $data[$key] = $val ? date('Y-m-d', $val) : $val;
                        } elseif ('datetime' == $type && is_int($val)) {
                            // 时间型
